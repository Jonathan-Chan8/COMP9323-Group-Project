...
def worker_shift_requests():
    ...
    shifts = Shifts.query.filter_by(shiftStatus = 'pending', requestedFrom = 'client', workerId = user_id).all()
    
    for shift in shifts:
        print(shift)
    
    return render_template('worker_shift_requests.html', user=user)
